...
<script>
export default {
  props: ['friendId', 'userId', 'chatId'], // Добавлены userId и chatId в props для использования в запросах
  data() {
    return {
      newMessage: '',
      errors: '',
  ...
  methods: {
    ...
    updateTyping() {
      clearTimeout(this.typingTimer);
      this.typingTimer = setTimeout(() => {
        Echo.private('chat.' + this.$chatId).whisper('typing', { typing: false });
      }, 2000);
      Echo.private('chat.' + this.$chatId).whisper('typing', { typing: true });
    },
    triggerClickPhoto(){
